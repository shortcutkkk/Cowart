...
Cowart 不会在首次启动 MCP 时自动安装依赖。若 MCP 报告缺少依赖，请在实际插件安装目录手动运行 `npm ci`，再重新启动 Codex。
...
Cowart 第一次启动 MCP 时会检查依赖；如果缺少 `tldraw` 等包，会显示实际插件目录和手动执行 `npm ci` 的提示。依赖安装始终由用户显式执行。
